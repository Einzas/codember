const decode = (text) => {
  ...
  const replit = Object.values(obj);
  const replit2 = decoded.filter((value, index) => {
    return decoded.indexOf(value) === index;
  });

  let final = "";
  for (let i = 0; i < replit.length; i++) {
    final += replit2[i] + replit[i];
  }
  console.log(final);
};
